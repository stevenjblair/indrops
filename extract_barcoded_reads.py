...
class IndropsLibrary():
    ...
    def quantify_expression_for_barcode(self, barcode, counts_output_filename, metrics_output_filename,
        ...
        script_dir = os.path.dirname(os.path.realpath(__file__))
        quant_cmd = [self.project.paths.python, os.path.sep.join([script_dir,"quantify_umifm_from_alignments.test.py"]), #self.project.paths.quantify_umifm_from_alignments_py,
        ...
        """
        ## debugging
        print("printing barcode reads for barcode: {}".format(barcode))
        ...
        # bowtie /home/unix/bhaas/regevtmp/bhaas/axoSC/InDropApril2017/Axo.Trinity.Mar2014.min10fpkm.fa -q - -p 1 -a --best --strata --chunkmbs 1000 --norc --sam --sam-nohead -shmem -m 200 -n 1 -l 15 -e 100
        

        reads_filename = aligned_bam + ".reads"
        reads_ofh = open(reads_filename, 'w')
        ...
        sys.stderr.write(" ".join(bowtie_cmd) + "\n")
        #p1 = subprocess.Popen(" ".join(bowtie_cmd), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        p1 = subprocess.Popen(" ".join(bowtie_cmd), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)

        (p1.stdout, p1.stderr) = p1.communicate()
        ...
        if p1.wait() != 0:
            ...
            sys.stderr(p1.stderr)
            raise Exception('\n === Error on bowtie ===')

        sys.stderr.write("command finished")

        hits_file = reads_filename + ".sam"
        hits_ofh = open(hits_file, 'w')
        num_hits = 0
        hits = []
        for line in p1.stdout:
            #sys.stderr.write(line)
            num_hits += 1
            #alignments_pipe.write(line)
            hits.append(line)
            hits_ofh.write(line)
        ...
        if num_hits == 0:
            sys.stderr.write("No alignments reported for: {}\n".format(barcode))
            return False

        sys.stderr.write("Got {} hits for barcode {}\n".format(num_hits, barcode))



        quant_cmd = "cat {} | {}".format(hits_file, " ".join(quant_cmd))
        sys.stderr.write(quant_cmd + "\n")

        # ex.
        # cat /home/unix/bhaas/regevtmp/bhaas/axoSC/InDropApril2017/iworm_asm/S_1/quant_dir/bcAAIE.aligned.bam.reads.sam | python /ahg/regev/users/bhaas/seq/bhaas/GIT/indrops/quantify_umifm_from_alignments.test.py -m 10 -u 2 -d 600 --min_non_polyA 15 --library S_1 --barcode bcAAIE --counts /home/unix/bhaas/regevtmp/bhaas/axoSC/InDropApril2017/iworm_asm/S_1/quant_dir/worker1_10.counts.tsv --metrics /home/unix/bhaas/regevtmp/bhaas/axoSC/InDropApril2017/iworm_asm/S_1/quant_dir/worker1_10.metrics.tsv --ambigs /home/unix/bhaas/regevtmp/bhaas/axoSC/InDropApril2017/iworm_asm/S_1/quant_dir/worker1_10.ambig.counts.tsv --ambig-partners /home/unix/bhaas/regevtmp/bhaas/axoSC/InDropApril2017/iworm_asm/S_1/quant_dir/worker1_10.ambig.partners --min-counts 0

        
        #sys.stderr.write(" ".join(quant_cmd) + "\n")
        #p2 = subprocess.Popen(quant_cmd, stdin=p1.stdout, stderr=subprocess.PIPE)
        p2 = subprocess.Popen(quant_cmd, stdin=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)

        (p2.stdout, p2.stderr) = p2.communicate()
...
class LibrarySequencingPart():
    ...
    def get_reads_for_barcode(self, barcode):
        ...
        with open(self.sorted_gzipped_fastq_filename, 'rb') as sorted_output:
            ...
            """
            ofh.write(byte_buffer.read())
            ofh.close()
            sys.stderr.write("wrote bytes file\n")
            sorted_output.seek(start_byte_offset)
            byte_buffer = BytesIO(sorted_output.read(byte_length))
            """
            ...
            all_unzipped = []
            #while True:
            #    all_unzipped.append(next(ungzipper))
            counter = 0
            for line in ungzipper:
                #sys.stderr.write(line)
                all_unzipped.append(line)
                counter += 1

            num_records = counter / 4
            sys.stderr.write("Identified {} reads for barcode {}\n".format(num_records, barcode))
...
if __name__=="__main__":
    ...
    if args.command == 'info':
        ...
        for library in target_libraries:
            sys.stderr.write("Running aggregation step for library {}\n".format(library))
            project.libraries[library].aggregate_counts(analysis_prefix=args.analysis_prefix)
